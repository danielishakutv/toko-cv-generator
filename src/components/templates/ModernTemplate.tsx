...
export function ModernTemplate({ data, template }: TemplateProps) {
  ...
  return (
    <div
      className={`bg-white w-[210mm] min-h-[297mm] mx-auto shadow-xl ${fontFamily}`}
      style={{ pageBreakAfter: 'always' }}
    >
      {/* Header with colored background */}
      <header className="p-12 pb-8" style={{ backgroundColor: `${primaryColor}15` }}>
        <h1 className="text-4xl font-bold mb-2" style={{ color: primaryColor }}>
          {data.personal.fullName}
        </h1>
        {data.personal.role && (
          <h2 className="text-xl text-gray-700 mb-4">{data.personal.role}</h2>
        )}
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm text-gray-700">
          ...
          {data.personal.socials?.map((social, idx) => (
            ...
            >
              {social.label.toLowerCase().includes('linkedin') && <Linkedin className="h-4 w-4" style={{ color: primaryColor }} />}
              {social.label.toLowerCase().includes('github') && <Github className="h-4 w-4" style={{ color: primaryColor }} />}
      ...
      {/* Two-column layout */}
      <div className="grid grid-cols-3 gap-8 p-12 pt-8">
        {/* Left column - Skills, Languages, etc. */}
                  ...
                  <div key={idx} className="text-sm">
                    ...
                    {skill.level && (
                      <div className="flex gap-1 mt-1">
                        {['Beginner', 'Intermediate', 'Advanced'].map((_, i) => (
                          <div
                            key={i}
                            className="h-1.5 w-full rounded-full"
                            style={{
                              backgroundColor:
                                skill.level === 'Advanced' && i <= 2
                                  ? primaryColor
              ...
              <div className="space-y-2 text-sm">
                {data.languages.map((lang, idx) => (
                  <div key={idx}>
                    ...
                    {lang.level && <div className="text-gray-600">{lang.level}</div>}
                  </div>
                ))}
              </div>
            </section>
              ...
              <div className="space-y-3">
                {data.education.map((edu, idx) => (
                  <div key={idx}>
                    ...
                    {edu.details && <p className="text-xs text-gray-700 mt-1">{edu.details}</p>}
                  </div>
                ))}
              </div>
            </section>
